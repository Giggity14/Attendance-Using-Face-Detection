    ...
    def face_recog(self):
        def draw_boundaries(img, classifier, scalefactor, minNeighbors, clf):
            ...
            for (x, y, w, h) in features:
                ...
                if confidence > 77:
                    cv2.putText(img, f"ID: {student_id}", (x, y - 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                    cv2.putText(img, f"Name: {name}", (x, y - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                    cv2.putText(img, f"Dept: {department}", (x, y - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
